...
module('Integration | Component | imgix image wrapped', function(hooks) {
  ...
  test('it renders event more better', async function(assert) {
    await render(hbs`<div style='width:200px;height:200px;'>{{imgix-image-wrapped path='/users/1.png' }}</div>`);
  ...
  test('it renders the correct path', async function(assert) {
    await render(hbs`<div style='width:1250px;'>{{imgix-image-wrapped path="/users/1.png"}}</div>`);
  ...
  test('it maintains any query parameters passed in', async function(assert) {
    assert.expect(2);
    await render(hbs`<div style='width:1250px;'>{{imgix-image-wrapped path="/users/1.png?sat=100"}}</div>`);
  ...
  test('it renders with an aspect ratio', async function(assert) {
    await render(hbs`<div style='width:1250px;'>{{imgix-image-wrapped path="/users/1.png" aspectRatio=1.3333}}</div>`);
  ...
  test('it respects passed in `crop` and `fit` values', async function(assert) {
    assert.expect(2);
    await render(hbs`{{imgix-image-wrapped path="/users/1.png?sat=100&fit=min&crop=top,left"}}`);
  ...
  test('it respects `crop` and `fit` values passed as attributes', async function(assert) {
    assert.expect(2);
    await render(hbs`{{imgix-image-wrapped path="/users/1.png" crop="top,left" fit="min"}}`);
  ...
  test('it respects `auto` values passed as attributes', async function(assert) {
    assert.expect(1);
    await render(hbs`{{imgix-image-wrapped path="/users/1.png" auto="compress,enhance"}}`);
  ...
  test('setting disableLibraryParam should cause the url not to contain an ixlib parameter', async function(assert) {
    await render(hbs`{{imgix-image-wrapped path="/users/1.png" disableLibraryParam=true}}`);
